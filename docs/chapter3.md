...
| US21 | Visualización de indicadores de impacto | Como visitante, deseo ver estadísticas y métricas del mercado logístico, para sentir confianza en la magnitud de la red. | **Scenario 1: Visualización de métricas**<br>_Given_ que el visitante explora la página principal<br>_When_ llega a la sección de métricas<br>_Then_ el sistema muestra indicadores numéricos de desempeño y alcance. | EP06 |
| US22 | Accesos directos de registro | Como visitante, deseo tener botones de acción rápidos (Call to Action) en la sección principal, para iniciar mi proceso de registro como empresa o transportista. | **Scenario 1: Uso del Call to Action**<br>_Given_ que el visitante se encuentra en la sección principal (Hero)<br>_When_ hace clic en el botón de registro<br>_Then_ el sistema lo redirige al formulario de registro correspondiente en la Web Application. | EP06 |
| US23 | Visualización de beneficios por segmento | Como visitante (transportista), deseo leer los beneficios específicos para conductores, para evaluar si me conviene unirme a la red. | **Scenario 1: Lectura de beneficios**<br>_Given_ que el visitante navega por el Landing Page<br>_When_ llega a la sección de beneficios para transportistas<br>_Then_ el sistema muestra las ventajas como mayor cantidad de fletes y pagos seguros. | EP06 |
| US24 | Redirección a inicio de sesión | Como visitante, deseo encontrar un acceso claro para iniciar sesión, para ingresar a mi cuenta si ya soy un usuario registrado. | **Scenario 1: Acceso al Login**<br>_Given_ que el visitante ya posee una cuenta<br>_When_ hace clic en el botón de 'Iniciar Sesión' en la barra de navegación<br>_Then_ el sistema lo redirige a la pantalla de autenticación de la aplicación web. | EP06 |
...
| 3 | US21 | Visualización de indicadores de impacto | Como visitante, deseo ver estadísticas y métricas del mercado logístico, para sentir confianza en la magnitud de la red. | 2 |
| 4 | US22 | Accesos directos de registro | Como visitante, deseo tener botones de acción rápidos (Call to Action) en la sección principal, para iniciar mi proceso de registro como empresa o transportista. | 2 |
| 5 | US23 | Visualización de beneficios por segmento | Como visitante (transportista), deseo leer los beneficios específicos para conductores, para evaluar si me conviene unirme a la red. | 2 |
| 6 | US24 | Redirección a inicio de sesión | Como visitante, deseo encontrar un acceso claro para iniciar sesión, para ingresar a mi cuenta si ya soy un usuario registrado. | 1 |
| 7 | US04 | Creación de solicitud de carga | Como dueño de negocio, deseo publicar una nueva solicitud indicando origen, destino y tipo de carga, para encontrar un transportista disponible. | 5 |
| 8 | US06 | Búsqueda de fletes disponibles | Como transportista, deseo visualizar una lista de solicitudes de carga cercanas, para seleccionar viajes compatibles con mi camión. | 5 |
| 9 | US07 | Aceptación de viaje | Como transportista, deseo aceptar una solicitud de carga, para asegurar mi participación en el servicio y obtener los datos de recojo. | 5 |
| 10 | US08 | Actualización de estado del viaje | Como transportista, deseo actualizar el estado del servicio (En camino, Recogido, Entregado), para mantener informada a la empresa contratante. | 3 |
| 11 | US09 | Seguimiento de carga | Como dueño de negocio, deseo visualizar el estado actualizado de mi viaje, para saber si la mercadería llegará a tiempo. | 3 |
| 12 | US16 | Endpoint de creación de viaje | Como Developer, deseo implementar un endpoint POST de solicitudes de carga, para recibir y almacenar los fletes en la BD. | 3 |
| 13 | US17 | Endpoint de matching geoespacial | Como Developer, deseo implementar un endpoint GET de viajes cercanos, para alimentar el catálogo del transportista. | 5 |
| 14 | US01 | Registro de Empresa | Como dueño de negocio, deseo registrar mi empresa en la plataforma, para poder publicar solicitudes de transporte. | 3 |
| 15 | US02 | Registro de Transportista | Como transportista independiente, deseo registrar mi perfil y vehículo, para acceder a nuevas oportunidades de fletes. | 3 |
| 16 | US03 | Inicio de sesión | Como usuario, deseo iniciar sesión con mis credenciales, para acceder a mi panel de gestión. | 2 |
| 17 | US15 | Autenticación basada en JWT | Como Developer, deseo implementar autenticación JWT en la API, para proteger los datos de empresas y transportistas. | 3 |
| 18 | US18 | Regla de validación de documentos | Como Developer, deseo configurar un proceso asíncrono que impida a transportistas no validados aceptar viajes, para garantizar la seguridad. | 5 |
| 19 | US10 | Calificación del transportista | Como dueño de negocio, deseo calificar el servicio del transportista, para construir confianza en la comunidad de la plataforma. | 2 |
| 20 | US11 | Historial de servicios | Como transportista, deseo revisar mi historial de viajes completados, para llevar un control de mis ingresos y métricas. | 2 |
| 21 | US05 | Cancelación de solicitud | Como dueño de negocio, deseo cancelar una solicitud de transporte no asignada, para evitar cobros si mis planes logísticos cambian. | 2 |
| 22 | US19 | Validación automática de transportista | Como Developer, quiero validar automáticamente la placa del vehículo contra el padrón del MTC y verificar formato/vigencia de los documentos, para habilitar o rechazar al transportista sin intervención manual. | 5 |
| 23 | US20 | Pago al transportista por servicio completado | Como dueño de negocio, quiero pagar al transportista con tarjeta una vez que el servicio ha sido entregado, para completar la transacción del viaje contratado. | 5 |
